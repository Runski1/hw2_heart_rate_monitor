...
class HeartLine:
    def __init__(self, pin):
        ...
        self.x_add = round(
            oled_width / self.pixel_range
        )  # next x is relative to drawing resolution
    ...
    def scale_data(self):
        ...
        scaled = round(
            ((value - self.minval) / (self.maxval - self.minval)) * self.scale_factor
        )
        return scaled
    ...
    def display(self):
        ...
        for _ in range(
            self.pixel_range
        ):  # amount of values per pixel. (Max 1 value per pixel)
            ...
            oled.line(
                x, y1, x + self.x_add, y2, 1
            )  # drawing a line from value to value
            oled.show()
...
class bpmCalc:
    def __init__(self):
        ...
        self.lastbpm = 85  # used to filter out unnatural fluctuations
        ...
        self.fifo = Fifo(3000, typecode="i")
    ...
    def detect(self, average):  # main algorythm
        while self.fifo.has_data():  # Run only when fifo not empty.
            ...
            if (
                self.lastval < self.threshold <= newval
            ):  # basic rising edge peak detection
                if (
                    self.minbpm > self.ms_counter > self.maxbpm
                ):  # skip values outside human heart rate.
                    seconds = self.ms_counter / 1000  # caclulate seconds between peaks
                    bpm = 60 // seconds  # calculate BPM
                    if (
                        abs(self.lastbpm - bpm) < 20
                    ):  # Ignore fluctuations of more than 10 BPM between beats
                        self.last_bpm = bpm
    ...
    def bpm_show(self):
        ...
        if self.average_bpm != 0:
            oled.text(f"{self.average_bpm}", 56, 40, 1)  # Show average BPM
        else:
            oled.text("--", 56, 38, 1)  # Show two lines when no BPM data available
        # Check if last average BPM is same as previous
        if self.last_average_bpm == self.average_bpm:
            self.reset_counter += 1
            if (
                self.reset_counter > 2
            ):  # If average BPM same for more than 5 times, reset values
...
def display_bpm(encoder):
    bpm = bpmCalc()
    heartline = HeartLine(27)  # 27 for sensor pin
